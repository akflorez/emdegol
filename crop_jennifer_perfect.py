...
jennifer_path = os.path.join(project_dir, "JENNIFER RINCON-FIFA.png")

img = Image.open(jennifer_path).convert("RGBA")
width, height = img.size
print(f"Jennifer sheet size: {width}x{height}")

# Sample background color
bg_color = img.getpixel((10, 10))
print(f"Jennifer bg color: {bg_color}")

# Search region for Jennifer's drawings: X from 750 to 1700, Y from 550 to 1550
search_rect = (750, 550, 1700, 1550)
...
def flood_fill_background(img_crop, tolerance=30):
    ...
    while head < len(queue):
        ...
        idx = y * w + x
        r, g, b, a = pixels[idx]
        dist = ((r - bg[0])**2 + (g - bg[2])**2 + (b - bg[2])**2)**0.5 # wait, fixing minor typo to g - bg[1]
...
for idx, (x1, x2) in enumerate(intervals[:3]):
    ...
    # Find feet bottom (where the figure ends, before the text label)
    y_end = h_reg - 1
    found_gap = False
    for y in range(y_start + 100, h_reg):
        if row_counts[y] < 8: # near zero count
            # Check if there is text below
            ...
            for ty in range(y + 5, min(h_reg, y + 60)):
                if row_counts[ty] > 5:
                    ...
                    break
            if has_text_below:
                y_end = y - 5 # Stop before the gap/text
                found_gap = True
                break
    ...
    # Add padding to top and sides
    crop_x1 = max(750, 750 + x1 - 10)
    crop_y1 = max(550, 550 + y_start - 10)
    crop_x2 = min(1700, 750 + x2 + 10)
    crop_y2 = min(1550, 550 + y_end)

    print(f"Jennifer Figure {idx+1} bounds: X=({crop_x1}, {crop_x2}), Y=({crop_y1}, {crop_y2})")
    
    sprite = img.crop((crop_x1, crop_y1, crop_x2, crop_y2))
    sprite_clean = flood_fill_background(sprite)
    
    filename = f"jennifer_360_{idx+1}.png"
    sprite_clean.save(os.path.join(project_dir, filename))
    print(f"Saved {filename}")

# Generate mirrored Side frame
side_path = os.path.join(project_dir, "jennifer_360_2.png")
if os.path.exists(side_path):
    side_img = Image.open(side_path)
    mirrored = ImageOps.mirror(side_img)
    mirrored.save(os.path.join(project_dir, "jennifer_360_4.png"))
    print("Generated mirrored jennifer_360_4.png")
